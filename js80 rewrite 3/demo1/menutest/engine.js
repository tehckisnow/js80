const engine = {
  ...
  menu: {
    newManager: function(game){
      ...
      manager.game = game;
      manager.menus = [];
      manager.newMenu = function(title, options, theme, themeMods){
        let newMenu = engine.menu.newMenu(game, title, options, theme, themeMods);
        manager.menus.push(newMenu);
        return newMenu;
      };
      //!closest most recent menu (should this be renamed?)
      manager.closeMenu = function(){
        manager.menus.pop();
      };
      manager.update = function(){
        for(m in manager.menus){
          //!if(manager.menus[m].active){}
          manager.menus[m].draw(manager.game);
       };
      };
      return manager;
    },
    newMenu: function(game, title, options, theme, themeMods){
      let menu = {};
      menu.currentOption = 0;
      menu.game = game;
      menu.title = title;
      menu.rawOptions = options;
      
      menu.theme = theme;
      menu.active = false;
      //! iterate themeMods

      engine.menu.updateMenuOptions(menu);
      menu.draw = function(){engine.menu.draw(game, menu)};
      menu.open = function(){
        menu.active = true;
      };
      menu.close = function(){
    ...
    updateMenuOptions: function(menu){
      let leftMargin = menu.theme.leftMargin;
      let topMargin = menu.theme.topMargin;
      let verticalSpacing = menu.theme.verticalSpacing;
      let headerSpace = verticalSpacing;
      if(menu.title == ""){headerSpace = 0};
      let currentPosition = 0;
      menu.options = [];
      for(i in menu.rawOptions){
        let curr = menu.rawOptions[i];
        //! if vertical:
        let currentY = (verticalSpacing * currentPosition) + topMargin + headerSpace + menu.theme.spacingAfterHeader;
        let currentX = leftMargin;
        menu.options.push(engine.menu.newOption(curr.text, curr.effect, currentX, currentY));
        currentPosition++;
      };
    },//menu.updateMenuOptions()
    
    newOption: function(text, effect, x, y){
      let option = {};
      option.text = text;
    ...
    newTheme: function(options){
      ...
      theme.leftMargin = 0;
      theme.topMargin = 0;
      theme.verticalSpacing = 10;
      theme.headerSpace = 10;
      for(o in options){
        theme[0] = options[0];
      };
      return theme;
    ...
    draw: function(game, menu){
      engine.render.rect(game, menu.theme.x, menu.theme.y, menu.theme.width, menu.theme.height);
